...
import { getBotFromStorage, saveBotToStorage } from "../actions/bot";
import { uploadDocument, uploadVacanciesToVectorStore } from "../actions/document";
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const existingBot = getBotFromStorage();
        const existingFileId =
          localStorage.getItem("job_openai_file_id") || undefined;

        // Upload do currículo para OpenAI (cria ou reutiliza o vector store)
        setStep("uploading");
        const uploaded = await uploadDocument(
          file,
          existingBot?.vectorStoreId,
          existingFileId,
          "Currículo - Site Vagas"
        );

        const botId = existingBot?.id || uuidv4();
        saveBotToStorage(botId, uploaded.vectorStoreId);
        localStorage.setItem("job_openai_file_id", uploaded.openaiFileId);
        localStorage.setItem("job_file_name", file.name);

        // Criar sessão local
        setStep("creating_session");
        const session = createSession(botId, clientId, "Candidato");
        localStorage.setItem("job_session_id", session.sessionId);

        // Extrai análise completa do currículo
        setStep("extracting");
        const resumeInfo = await extractResumeInfo(uploaded.vectorStoreId);
        console.log(
          "[extractResumeInfo] resultado (primeiros 500 chars):",
          resumeInfo.slice(0, 500)
        );

        if (!resumeInfo || resumeInfo.trim().length < 50) {
          console.warn("[extractResumeInfo] resposta muito curta, tentando novamente...");
          await new Promise((resolve) => setTimeout(resolve, 5000));
          const retry = await extractResumeInfo(uploaded.vectorStoreId);
          localStorage.setItem("job_resume_info", retry || resumeInfo);
        } else {
          localStorage.setItem("job_resume_info", resumeInfo);
        }

        const finalResumeInfo =
          localStorage.getItem("job_resume_info") || resumeInfo;

        // Gerar query de busca
        setStep("building_query");
        const searchQuery = await buildVacancyQuery(
          uploaded.vectorStoreId,
          finalResumeInfo
        );
        console.log("[buildVacancyQuery] query:", searchQuery);

        // Redireciona para a interface com mapa em loading — Manus roda em background
        setStep("done");
        onComplete({
          botId,
        ...
        try {
          ...
          window.dispatchEvent(
            new CustomEvent("vacancies_ready", { detail: vacancies })
          );

          // Sobe o JSON das vagas no vector store para o chat ter contexto completo
          if (vacancies.length > 0) {
            try {
              await uploadVacanciesToVectorStore(uploaded.vectorStoreId, vacancies);
              console.log("[uploadVacanciesToVectorStore] vagas indexadas no vector store");
            } catch (uploadErr) {
              console.warn("[uploadVacanciesToVectorStore] falha ao indexar vagas:", uploadErr);
            }
          }
        } catch (searchErr) {
          console.error("[searchVacancies background]", searchErr);
          window.dispatchEvent(
            new CustomEvent("vacancies_ready", { detail: [] })
          );
        }
      } catch (err) {
        console.error(err);
          ...
          {!isLoading && step !== "done" ? (
            <>
              ...
              <p className="text-slate-400 text-sm text-center mb-6">
                Nossa IA irá analisar seu perfil e encontrar as vagas mais
                compatíveis
              </p>
                ...
                <div className="flex flex-col items-center gap-3">
                  <div
                    className={`w-14 h-14 rounded-full flex items-center justify-center transition-colors shrink-0 ${
                      dragOver ? "bg-purple-500/20" : "bg-slate-700"
                    }`}
                  >
                    <svg
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                {[
                  ...
                  <div
                    key={item.label}
                    className="bg-slate-700/40 rounded-lg p-3 text-center"
                  >
